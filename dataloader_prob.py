...
class MyDataset(Dataset):
    ...
    def __init__(self, rootdir, data_list, mode='whisper'):
        ...
        for line in f:
            ...
            category = [float(x) for x in category]
    ...
    def __getitem__(self, idx):
        ...
        if self.mode=='whisper':
            ...
            wav_en = torch.reshape(wav_en, (-1,))
            wav_en = processor(wav_en, sampling_rate=16000, return_tensors="pt").input_features           
...
class MyNoiseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #max_abs = torch.max(torch.abs(wav_en))
        #wav_en = wav_en/max_abs
