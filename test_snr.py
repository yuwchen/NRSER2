...
def main():
    ...
    parser.add_argument(
        "--cmgan_base_model",
        type=str,
        default="./CMGAN/best_ckpt/ckpt",
        help="Path to pretrained CMGAN model.",
    )
    parser.add_argument(
        "--datadir", type=str, default="./data", help="Path of your DATA/ directory"
    )
    parser.add_argument("--datalist", type=str, help="Path of your DATA/ list")
    ...
    if torch.cuda.is_available():
        ...
        se_model.eval()
        se_model.load_state_dict(torch.load(se_path))
    else:
        se_model = generator.TSCNet(num_channel=64, num_features=N_FFT // 2 + 1).cpu()
        se_model.eval()
        se_model.load_state_dict(torch.load(se_path, map_location=torch.device("cpu")))

    noise_model = SNRLevelDetection(N_FFT, HOP).to(device)
    noise_model.eval()
    noise_model.load_state_dict(
        torch.load(os.path.join(my_checkpoint_dir, "NOISE" + os.sep + "best"))
    )

    print("Loading data")

    validset = open(datalist, "r").read().splitlines()
    outfile = my_checkpoint_dir.split("/")[-1] + "_" + datalist.split("/")[-1]
    ...
    if savewav:
        wav_output_dir = os.path.join(
            "Enhanced_wav", my_checkpoint_dir.split(os.sep)[-1]
        )
    ...
    for filepath in tqdm(validset):
        ...
        with torch.no_grad():
            ...
            snr_level_score = torch.clamp(S, min=0.0, max=1.0)

            snr_level_score = snr_level_score.cpu().detach().numpy()
            S = S.cpu().detach().numpy()

            torch.cuda.empty_cache()
            output = "{}; S_raw:{}; S:{}".format(filename, S, snr_level_score)
            print(output)
