...
from dataloader_prob import MyDataset, MyNoiseDataset
from emotion_model_whisper_en_prob import EmotionPredictor
...
def main():
    ...
    parser.add_argument('--pretrained_model_emotion', default='emotion_model_whisper_en_prob_r1', type=str, required=False, help='Path to your Emotion model directory')
    parser.add_argument('--pretrained_model_snr', default='snr_model_v3', type=str, required=False, help='Path to your SNR-level detection model directory')
    ...
    trainlist = os.path.join(txtfiledir, 'msp1_11-train-clean-noisy-8-class-prob.txt')
    validlist = os.path.join(txtfiledir, 'msp1_11-dev-clean-noisy-8-class-prob.txt')


    trainset = MyDataset(wavdir, trainlist, 'raw')
    trainloader = DataLoader(trainset, batch_size=8, shuffle=True, num_workers=2, collate_fn=trainset.collate_fn)
    validset = MyDataset(wavdir, validlist, 'raw')
    validloader = DataLoader(validset, batch_size=8, shuffle=True, num_workers=2, collate_fn=validset.collate_fn)

    trainlist_n = os.path.join(txtfiledir, 'snr-audioset-train-80-msp1_11-train-clean.txt')
    validlist_n = os.path.join(txtfiledir, 'snr-audioset-val-msp1_11-test2-clean.txt')
    ...
    emo_net = EmotionPredictor(ssl_model, SSL_OUT_DIM, device)
    ...
    snr_net.load_state_dict(torch.load(os.path.join(pretrained_model_snr,'NOISE'+os.sep+'best')))
    ...
    for epoch in range(1,1001):
        ...
        for i, data in enumerate(tqdm(trainloader), 0):
            ...
            emo_input = (inputs_en*(1-snr_level_score) + inputs*snr_level_score)/2

            processed_wavs = []
            for wav in emo_input:
                wav = wav.cpu().detach().numpy()
                wav = np.reshape(wav,(-1, ))
                wav = processor(wav, sampling_rate=16000, return_tensors="pt").input_features
                processed_wavs.append(wav)

            processed_wavs = torch.stack(processed_wavs, dim=0).squeeze(1).to(device)

            output_A, output_V, output_D, output_C = emo_net(processed_wavs)
